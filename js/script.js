...
const cardNumber = document.getElementsByClassName("num-box");
const zipCode = document.getElementsByClassName("zip-box");
const cvv = document.getElementsByClassName("cvv-box");
...
const registerValidator = () => {
  const registerSectionIsValid = registerForActivities.length > 0;
  if (registerSectionIsValid) {
    validationPass(registerForActivities);
    console.log(
      "Total number of activities registered: ",
      `"${registerForActivities.value}"`
    );
  } else {
...
const creditCardValidator = () => {
  const creditCardSectionIsValid = /^\d{13,16}/.test(creditCard.value);
  if (creditCardSectionIsValid) {
    validationPass(creditCard);
    console.log("Credit Card entered is: ", `"${creditCard.value}"`);
  } else {
    validationFail(creditCard);
...
const zipCodeValidator = () => {
  ...
  console.log(
    `Zip code section validation test evaluates to ${zipCodeSectionIsValid}`
  );
